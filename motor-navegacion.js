...
function exportarRutaGPX(id){
  ...
  // (rutas.js) trae el track exacto desde /routesTrack si es tuya (privacidad, hub #201).
  if(typeof _traerPuntosRuta==='function'){ _traerPuntosRuta(id, function(pts, doc){ armar(pts||[], (doc&&doc.nombreRuta)||'Ruta Libre Pedal'); }); return; }
  db.collection('routes').doc(id).get().then(function(doc){ if(doc.exists){ const dd=doc.data(); armar((dd.points||dd.pointsPub||[]), dd.nombreRuta||'Ruta Libre Pedal'); } else { lpAviso('No pude abrir esa ruta.'); } }).catch(function(){ lpAviso('No pude abrir esa ruta.'); });
}
