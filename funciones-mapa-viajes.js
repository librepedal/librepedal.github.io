...
function loadHostels(){ if(!loadHostels._votos){ loadHostels._votos=true; initVotosHostels(); } db.collection('hostels').orderBy('ts','desc').limit(50).onSnapshot(function(snapshot){ allHostels=[]; snapshot.forEach(function(doc){ const d=doc.data(); if(/^TEST-/.test(d.name||'')) return; allHostels.push(Object.assign({id:doc.id},d)); }); renderHostelsLista(); /* si el selector de "Nuevo viaje" está abierto en este momento (ver showNewTripForm), refresca también su lista -- si no, el fix de arriba solo dispara la carga pero la pantalla se queda con el "Sin hostels guardados" inicial hasta que el usuario cambia de filtro y vuelve. */ if(currentFilter==='hostel' && document.getElementById('dest-options')) renderHostelOptions(); }, function(){ const c=document.getElementById('hostel-list'); if(c) c.innerHTML='<p style="color:#7d8ba0;font-size:0.8rem;text-align:center;padding:20px 0">No se pudieron cargar los alojamientos. Revisa tu conexión.</p>'; }); }
...
// Auditoría 2026-09-23 (cumplimiento Play Store, antes de cualquier envío): este botón
// ("Marcar con GPS" al agregar un hospedaje) pedía navigator.geolocation.getCurrentPosition()
// DIRECTO, sin pasar por lpAsegurarUbicacion() -- el mismo tipo exacto de hueco que causó
// el rechazo real "Missing Prominent Disclosure". Era alcanzable como el PRIMER contacto de
// un usuario con cualquier cosa de ubicación (Comunidad -> Alojamientos -> Agregar ->
// "Marcar con GPS"), disparando el diálogo nativo sin ningún aviso propio antes. El click
// para marcar el punto A MANO en el mapa (más abajo) no pide permiso, así que se deja intacto.
async function toggleHostelMap(){ const mapDiv=document.getElementById('map-hostel-select'); const st=document.getElementById('gpsStatus'); if(mapDiv.classList.contains('show')){ mapDiv.classList.remove('show'); st.classList.remove('show'); return; } mapDiv.classList.add('show'); st.classList.add('show'); if(!mapHostelSelect){ mapHostelSelect=L.map('map-hostel-select').setView([-34.6037,-58.3816],13); L.tileLayer('https://a.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(mapHostelSelect); } mapHostelSelect.on('click',function(e){ hostelCoords={lat:e.latlng.lat,lon:e.latlng.lng}; if(mapHostelSelect._um) mapHostelSelect.removeLayer(mapHostelSelect._um); mapHostelSelect._um=L.marker([e.latlng.lat,e.latlng.lng]).addTo(mapHostelSelect); st.innerText='Punto: '+e.latlng.lat.toFixed(5)+', '+e.latlng.lng.toFixed(5); }); setTimeout(function(){ mapHostelSelect.invalidateSize(); },300); if(!navigator.geolocation){ st.innerText='GPS no disponible. Toca el mapa para marcar el punto.'; return; } st.innerText='Toca el mapa para marcar el punto, o espera tu ubicación...'; if(!(await lpAsegurarUbicacion())) return; navigator.geolocation.getCurrentPosition(function(pos){ const lat=pos.coords.latitude, lon=pos.coords.longitude; hostelCoords={lat:lat,lon:lon}; mapHostelSelect.setView([lat,lon],16); if(mapHostelSelect._um) mapHostelSelect.removeLayer(mapHostelSelect._um); mapHostelSelect._um=L.marker([lat,lon]).addTo(mapHostelSelect); st.innerText='Ubicación: '+lat.toFixed(5)+', '+lon.toFixed(5); }, function(err){ st.innerText='No se pudo obtener tu ubicación automática ('+err.message+'). Toca el mapa para marcar el punto a mano.'; }, {enableHighAccuracy:true,timeout:10000}); }
...
// Auditoría 2026-09-22: allHostels solo se llenaba via _subUnaVez('hostels', loadHostels)
// desde abrirTeDoyAlojo() -- una función oculta tras 6 meses de antigüedad que casi nadie
// alcanza a abrir. Resultado real: el selector de hospedajes de "Nuevo viaje" (abajo,
// filterDestType('hostel',...) -> renderHostelOptions(), que lee allHostels) aparecía
// VACÍO ("Sin hostels guardados") para casi todos los usuarios, aunque sí hubiera
// alojamientos reales publicados. Se agrega la misma suscripción perezosa acá también
// (_subUnaVez es idempotente por clave -- si "Te doy alojo" ya la abrió antes, esto no
// hace nada de más).
function showNewTripForm(){ addedDests=[]; document.getElementById('trip-name').value=''; document.getElementById('trip-start').value=''; renderAddedDests(); _subUnaVez('hostels', loadHostels); filterDestType('hostel',document.querySelector('#v-newtrip .gtt')); cv('newtrip'); try{ if(typeof _mantencionAvisoPreViaje==='function') _mantencionAvisoPreViaje(); }catch(e){} try{ if(typeof _vehAvisoPreViaje==='function') _vehAvisoPreViaje(); }catch(e){} }
...
// 2026-09-22: loadTrips() se llama al iniciar sesión Y de nuevo al final de CADA
// endNavigation() (para refrescar "Mis viajes" apenas termina un viaje planificado) --
// sin guardia, cada una de esas llamadas abría un onSnapshot NUEVO sin cerrar el
// anterior: misma clase de fuga que ya había tumbado la app por cuota (ver
// suscripcion-perezosa-rtd.js). Se envuelve el cuerpo real en _subUnaVez para que solo
// exista UN listener vivo por sesión, sin tocar los call-sites (siguen escribiendo
// loadTrips() tal cual, incluido el arranque -- ver lecturas-firestore.test.mjs, que
// exige loadTrips() como llamada directa en las dos ramas de login). Como el listener
// ya queda suscrito desde el login, cuando endNavigation() actualiza el status de un
// trip a 'completed' el propio onSnapshot recibe ese cambio en tiempo real -- llamar
// loadTrips() de nuevo ahí ya era redundante, esto solo lo vuelve inofensivo.
function loadTrips(){
  _subUnaVez('trips', function(){
    db.collection('trips').where('user','==',cu).onSnapshot(function(snapshot){ trips=[]; snapshot.forEach(function(doc){ trips.push(Object.assign({id:doc.id},doc.data())); }); trips.sort(function(a,b){ return ((b.createdAt&&b.createdAt.seconds)||0)-((a.createdAt&&a.createdAt.seconds)||0); }); renderTrips(); }, function(){ const c=document.getElementById('trips-list'); if(c) c.innerHTML='<p style="color:#7d8ba0;font-size:0.8rem;text-align:center;padding:20px 0">No se pudieron cargar tus viajes. Revisa tu conexión.</p>'; });
  });
}
